...
void InterfacesWidget_C::InterfaceSelected(const QModelIndex& Index)
{
	...
	QString	UuidStringUUID = pProxyModel->data(pProxyModel->index(Index.row(), Column_Uuid)).toString();
	...
	CLSIDFromString(UuidStringUUID.toStdWString().c_str(), &RpcIfId.Uuid);
...
bool InterfacesWidget_C::IsInterfacePresent(quint32 Pid, RPC_IF_ID* pIfId)
{
	...
	if ( StringFromCLSID(pIfId->Uuid,&pUuidString)!=RPC_S_OK ) goto End;

	ItemList = pModel->findItems(QString::fromStdWString(pUuidString), Qt::MatchFixedString, Column_Uuid);
...
bool InterfacesWidget_C::AddInterfaces(RpcInterfaceInfo_T* pRpcInterfaceInfo)
{
	...
	if ( StringFromCLSID(pRpcInterfaceInfo->If.Uuid,&pUuidString)!=RPC_S_OK ) goto End;
	...
	pModel->setData(pModel->index(Index, Column_Uuid), QString::fromStdWString(pUuidString));
